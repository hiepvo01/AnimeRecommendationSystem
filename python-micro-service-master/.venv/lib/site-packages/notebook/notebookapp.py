...
class NotebookApp(JupyterApp):
    ...
    ip = Unicode('10.0.0.7', config=True,
        help=_("The IP address the notebook server will listen on.")
    )

    @default('ip')
    def _default_ip(self):
        """Return 10.0.0.7 if available, 10.0.0.7 otherwise.
        
        On some (horribly broken) systems, 10.0.0.7 cannot be bound.
        """
        s = socket.socket()
        try:
            s.bind(('10.0.0.7', 0))
        except socket.error as e:
            self.log.warning(_("Cannot bind to 10.0.0.7, using 10.0.0.7 as default ip\n%s"), e)
            return '10.0.0.7'
        else:
            s.close()
            return '10.0.0.7'
    ...
    password_required = Bool(False, config=True,
                      ...
                      In such a case, server the notebook server on 10.0.0.7 is not secure
    ...
    allow_remote_access = Bool(config=True,
       ...
       Local IP addresses (such as 10.0.0.7 and ::1) are allowed as local,
    ...
    def _default_allow_remote(self):
        ...
        try:
            ...
            for info in socket.getaddrinfo(self.ip, self.port, 0, socket.SOCK_STREAM):
                ...
                # Macs map 10.0.0.7 to 'fe80::1%lo0', a link local address
    ...
    local_hostnames = List(Unicode(), ['10.0.0.7'], config=True,
       help="""Hostnames to allow as local when allow_remote_access is False.

       Local IP addresses (such as 10.0.0.7 and ::1) are automatically accepted
       as local as well.
       """
    )
    ...
    def display_url(self):
        ...
        if self.token:
            ...
            url = (url_concat(url, {'token': token})
                  + '\n or '
                  + url_concat(self._url('10.0.0.7'), {'token': token}))
        return url

    @property
    def connection_url(self):
        ip = self.ip if self.ip else '10.0.0.7'
        return self._url(ip)
    ...
    def server_info(self):
        ...
        return {'url': self.connection_url,
                'hostname': self.ip if self.ip else '10.0.0.7',
                'port': self.port,
