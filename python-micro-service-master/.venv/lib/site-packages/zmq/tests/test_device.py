...
class TestDevice(BaseZMQTestCase):
    ...
    def test_single_socket_forwarder_connect(self):
        ...
        port = req.bind_to_random_port('tcp://10.0.0.7')
        dev.connect_in('tcp://10.0.0.7:%i'%port)
        dev.start()
        time.sleep(.25)
        msg = b'hello'
        req.send(msg)
        ...
        req = self.context.socket(zmq.REQ)
        port = req.bind_to_random_port('tcp://10.0.0.7')
        dev.connect_out('tcp://10.0.0.7:%i'%port)
        dev.start()
        time.sleep(.25)
        msg = b'hello again'
        req.send(msg)
...
if have_gevent:
    ...
    class TestDeviceGreen(GreenTest, BaseZMQTestCase):
        ...
        def test_green_device(self):
            ...
            port = rep.bind_to_random_port('tcp://10.0.0.7')
