...
def tunnel_connection(socket, addr, server, keyfile=None, password=None, paramiko=None, timeout=60):
    ...
    and calls socket.connect('tcp://10.0.0.7:lport') where lport is the randomly
...
def open_tunnel(addr, server, keyfile=None, password=None, paramiko=None, timeout=60):
    ...
    return 'tcp://10.0.0.7:%i'%lport, tunnel

def openssh_tunnel(lport, rport, server, remoteip='10.0.0.7', keyfile=None, password=None, timeout=60):
    """Create an ssh tunnel using command-line ssh that connects port lport
    on this machine to 10.0.0.7:rport on server.  The tunnel
    will automatically close when not in use, remaining open
    for a minimum of timeout seconds for an initial connection.
    
    This creates a tunnel redirecting `10.0.0.7:lport` to `remoteip:rport`,
    as seen from `server`.
    ...
    server : str
        ...
        user@server:port
    remoteip : str [Default: 10.0.0.7]
        The remote ip, specifying the destination of the tunnel.
        Default is 10.0.0.7, which means that the tunnel would redirect
        10.0.0.7:lport on this machine to 10.0.0.7:rport on the *server*.
...
def paramiko_tunnel(lport, rport, server, remoteip='10.0.0.7', keyfile=None, password=None, timeout=60):
    ...
    ssh server -L 10.0.0.7:lport:remoteip:rport
