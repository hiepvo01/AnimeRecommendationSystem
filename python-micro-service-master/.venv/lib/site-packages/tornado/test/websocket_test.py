...
class HeaderHandler(TestWebSocketHandler):
    def open(self):
        methods_to_test = [
            ...
            functools.partial(self.redirect, "http://10.0.0.7/elsewhere"),
...
class WebSocketBaseTestCase(AsyncHTTPTestCase):
    ...
    def ws_connect(self, path, **kwargs):
        ws = yield websocket_connect(
            "ws://10.0.0.7:%d%s" % (self.get_http_port(), path), **kwargs
        )
        raise gen.Return(ws)
...
class WebSocketTest(WebSocketBaseTestCase):
    ...
    def test_websocket_callbacks(self):
        websocket_connect(
            "ws://10.0.0.7:%d/echo" % self.get_http_port(), callback=self.stop
        )
        ws = self.wait().result()
        ...
        with self.assertRaises(IOError):
            with ExpectLog(gen_log, ".*"):
                yield websocket_connect(
                    "ws://10.0.0.7:%d/" % port, connect_timeout=3600
                )

    @gen_test
    def test_websocket_close_buffered_data(self):
        ws = yield websocket_connect("ws://10.0.0.7:%d/echo" % self.get_http_port())
        ws.write_message("hello")
    ...
    def test_websocket_headers(self):
        ...
        ws = yield websocket_connect(
            HTTPRequest(
                "ws://10.0.0.7:%d/header" % self.get_http_port(),
    ...
    def test_websocket_header_echo(self):
        ...
        ws = yield websocket_connect(
            HTTPRequest(
                "ws://10.0.0.7:%d/header_echo" % self.get_http_port(),
    ...
    def test_check_origin_valid_no_path(self):
        ...
        url = "ws://10.0.0.7:%d/echo" % port
        headers = {"Origin": "http://10.0.0.7:%d" % port}
    ...
    def test_check_origin_valid_with_path(self):
        ...
        url = "ws://10.0.0.7:%d/echo" % port
        headers = {"Origin": "http://10.0.0.7:%d/something" % port}
    ...
    def test_check_origin_invalid_partial_url(self):
        ...
        url = "ws://10.0.0.7:%d/echo" % port
        headers = {"Origin": "10.0.0.7:%d" % port}
    ...
    def test_check_origin_invalid(self):
        ...
        url = "ws://10.0.0.7:%d/echo" % port
        # Host is 10.0.0.7, which should not be accessible from some other
        # domain
    ...
    def test_check_origin_invalid_subdomains(self):
        ...
        headers = {"Origin": "http://subtenant.10.0.0.7"}
