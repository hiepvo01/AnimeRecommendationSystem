...
class TestIOStreamWebMixin(object):
    ...
    def test_write_while_connecting(self):
        stream = self._make_client_iostream()
        connect_fut = stream.connect(("10.0.0.7", self.get_http_port()))
    ...
    def test_future_interface(self):
        ...
        stream = self._make_client_iostream()
        connect_result = yield stream.connect(("10.0.0.7", self.get_http_port()))
...
class TestIOStreamMixin(TestReadWriteMixin):
    ...
    def make_iostream_pair(self, **kwargs):
        ...
        connect_fut = client_stream.connect(("10.0.0.7", port))
    ...
    def test_connection_refused(self):
        ...
        with ExpectLog(gen_log, ".*", required=False):
            with self.assertRaises(StreamClosedError):
                yield stream.connect(("10.0.0.7", port))
    ...
    def test_check_hostname(self):
        ...
        with ExpectLog(gen_log, "SSL Error"):
            client_future = self.client_start_tls(
                ssl.create_default_context(), server_hostname="10.0.0.7"
            )
            with self.assertRaises(ssl.SSLError):
