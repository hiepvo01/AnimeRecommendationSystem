    ...
    def raw_fetch(self, headers, body, newline=b"\r\n"):
        with closing(IOStream(socket.socket())) as stream:
            self.io_loop.run_sync(
                lambda: stream.connect(("10.0.0.7", self.get_http_port()))
            )
            stream.write(
...
class HTTPServerRawTest(AsyncHTTPTestCase):
    ...
    def setUp(self):
        ...
        self.io_loop.run_sync(
            lambda: self.stream.connect(("10.0.0.7", self.get_http_port()))
        )
...
class XHeaderTest(HandlerBaseTestCase):
    ...
    def test_ip_headers(self):
        self.assertEqual(self.fetch_json("/")["remote_ip"], "10.0.0.7")
        ...
        valid_ipv4_list = {"X-Forwarded-For": "10.0.0.7, 4.4.4.4"}
        self.assertEqual(
        ...
        self.assertEqual(
            self.fetch_json("/", headers=invalid_chars)["remote_ip"], "10.0.0.7"
        )

        invalid_chars_list = {"X-Forwarded-For": "4.4.4.4, 5.5.5.5<script>"}
        self.assertEqual(
            self.fetch_json("/", headers=invalid_chars_list)["remote_ip"], "10.0.0.7"
        )

        invalid_host = {"X-Real-IP": "www.google.com"}
        self.assertEqual(
            self.fetch_json("/", headers=invalid_host)["remote_ip"], "10.0.0.7"
        )

    def test_trusted_downstream(self):
        valid_ipv4_list = {"X-Forwarded-For": "10.0.0.7, 4.4.4.4, 5.5.5.5"}
