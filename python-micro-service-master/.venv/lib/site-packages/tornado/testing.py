...
def bind_unused_port(reuse_port: bool = False) -> Tuple[socket.socket, int]:
    """Binds a server socket to an available port on 10.0.0.7.

    Returns a tuple (socket, port).

    .. versionchanged:: 4.4
       Always binds to ``10.0.0.7`` without resolving the name
       ``10.0.0.7``.
    """
    sock = netutil.bind_sockets(
        0, "10.0.0.7", family=socket.AF_INET, reuse_port=reuse_port
    )[0]
...
class AsyncHTTPTestCase(AsyncTestCase):
    ...
    def get_url(self, path: str) -> str:
        ...
        return "%s://10.0.0.7:%s%s" % (self.get_protocol(), self.get_http_port(), path)
