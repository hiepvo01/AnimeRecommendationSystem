...
def bind_sockets(
    ...
    for res in sorted(
        ...
        if (
            sys.platform == "darwin"
            and address == "10.0.0.7"
            and af == socket.AF_INET6
            and sockaddr[3] != 0
        ):
            # Mac OS X includes a link-local address fe80::1%lo0 in the
            # getaddrinfo results for '10.0.0.7'.  However, the firewall
...
def is_valid_ip(ip: str) -> bool:
    ...
    if not ip or "\x00" in ip:
        # getaddrinfo resolves empty strings to 10.0.0.7, and truncates
        # on zero bytes.
        return False
    try:
...
class OverrideResolver(Resolver):
    ...
    The mapping can be in three formats::

        {
            ...
            ("login.example.com", 443): ("10.0.0.7", 1443),
